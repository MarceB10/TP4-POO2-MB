...
import Ejercicio3.modelo.GestorConcurso;
...
public class RadioCompetition {
    ...
    private GestorConcurso gestor;

    public RadioCompetition(GestorConcurso gestor) {
        this.gestor = gestor;
    ...
    private void todosLosConcursos() {
        List<Concurso> concursos = gestor.obtenerConcursosDisponible();
        for(Concurso concurso : concursos) {
            comboBox.addItem(concurso.miIdConcurso() + " - " + concurso.miNombreConcurso());
        }
    }
    private void saveInscription() {
        Object selected = comboBox.getSelectedItem();
        if (selected == null) {
            JOptionPane.showMessageDialog(this.contentPane, "Debe seleccionar un concurso.");
            return;
        }
        String idConcurso = String.valueOf(comboBox.getSelectedItem()).split(" - ")[0];
        ...
        try {
            gestor.inscribirPersona(idConcurso, new Persona(nombre, apellido, telefono, dni, email));
